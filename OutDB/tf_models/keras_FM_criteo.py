...
#del validation_data
    ...
    dfTrain= pd.read_csv("/data2/ruike/greenplum/cmd/criteo/train_process.csv")
    dfTest= pd.read_csv("/data2/ruike/greenplum/cmd/criteo/train_sample_process.csv")
    ID_columns  = ["c"+str(i) for i in range(14, 40)]#["ps_reg_01", "ps_reg_02", "ps_reg_03","ps_car_12", "ps_car_13", "ps_car_14", "ps_car_15",]
    target_columns = [str("label")]
    ...
    #del validation_data
    gc.collect()
    max_features = [1460, 575, 7382921, 1693085, 305, 24, 12348, 633, 3, 85971, 5560, 6136222, 3161, 27, 14442, 4079009, 10, 5490, 2167, 4, 5217290, 18, 15, 241688, 105, 127081]
...
model= build_model(max_features,continue_cols,K=12,solver='adam',l2=0.1,l2_fm = 0.15)
#print(model.summary())
f = open('model_json.txt','w')
